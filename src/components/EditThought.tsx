...
const EditThought: React.FC<EditThoughtProps> = ({ thoughtId, onClose, onSuccess, showAlert }) => {
  ...
  const fetchThoughtData = async () => {
    try {
      const response = await api.get(`/v1/thought/${thoughtId}`);
      ...
      showAlert('Failed to fetch thought data', 'error');
      onClose();
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/v1/thought/${thoughtId}`, formPayload, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });

      showAlert('Thought updated successfully!', 'success');
      onSuccess();
      onClose();
    } catch (error) {
      console.error('Error updating thought:', error);
      showAlert('Failed to update thought. Please try again.', 'error');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white border border-black shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-black px-6 py-4 flex justify-between items-center">
          <div className="flex items-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900">Edit Thought</h2>
          </div>
          <button
            onClick={onClose}
