...
const EditDoYouKnow: React.FC<EditDoYouKnowProps> = ({ doYouKnow, onClose, showAlert, onSuccess }) => {
  ...
  useEffect(() => {
    if (doYouKnow) {
      setFormData({
        ...
        content: doYouKnow.content || '',
        isActive: doYouKnow.isActive ?? true,
        expiry: doYouKnow.expiry ? new Date(doYouKnow.expiry).toISOString().slice(0, 16) : ''
      });
      if (doYouKnow.image) {
        setImagePreview(`https://laqsya.com/${doYouKnow.image}`);
      }
    }
  }, [doYouKnow]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`/v1/do-you-know/${doYouKnow._id}`, formDataToSend, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });

      showAlert('Do You Know updated successfully!', 'success');
      onSuccess();
      onClose();
    } catch (error: any) {
      showAlert(error.response?.data?.message || 'Failed to update Do You Know', 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60] p-4">
      <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-black">
        <div className="sticky top-0 bg-white border-b border-black p-6 flex justify-between items-center">
          <h3 className="text-xl font-bold">Edit Do You Know</h3>
