...
interface ViewDoYouKnowCommentsProps {
  doYouKnowId: string;
...
const ViewDoYouKnowComments: React.FC<ViewDoYouKnowCommentsProps> = ({
  doYouKnowId,
  doYouKnowTitle,
  onClose,
  showAlert
  ...
  const fetchComments = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`https://laqsya.com/api/v1/do-you-know/comment/${doYouKnowId}/comment`);
        ...
        <div className="sticky top-0 bg-white border-b border-black p-6 flex justify-between items-center z-10">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">{doYouKnowTitle}</p>
            </div>
          </div>
          <button
            onClick={onClose}
...
export default ViewDoYouKnowComments;
