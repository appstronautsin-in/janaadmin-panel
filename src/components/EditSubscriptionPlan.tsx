...
const EditSubscriptionPlan: React.FC<EditSubscriptionPlanProps> = ({ planId, onClose, onSuccess, showAlert }) => {
  ...
  const fetchPlanData = async () => {
    try {
      const response = await api.get(`/v1/admin/subscription/package/subscription-packages/${planId}`);
      const plan = response.data;
      
      setFormData({
        planName: plan.planName,
        ePaperAllowedInApp: plan.ePaperAllowedInApp,
        ePaperAllowedInWeb: plan.ePaperAllowedInWeb,
        price: plan.price.toString(),
        features: plan.features.length > 0 ? plan.features : [''],
        validity: plan.validity,
        newsAllowedInApp: plan.newsAllowedInApp,
        newsAllowedInWebsite: plan.newsAllowedInWebsite
      });
    } catch (error) {
      console.error('Error fetching plan data:', error);
      showAlert('Failed to fetch plan data', 'error');
      onClose();
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/v1/admin/subscription/package/subscription-packages/${planId}`, payload);
      showAlert('Subscription plan updated successfully!', 'success');
      onSuccess();
      onClose();
    } catch (error) {
      console.error('Error updating subscription plan:', error);
      showAlert('Failed to update subscription plan. Please try again.', 'error');
    } finally {
      setLoading(false);
    }
  };
